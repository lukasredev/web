...
- Automatic image tagging strategies (including semantic versioning)
...
Create a new file for the Docker build workflow:
...
- `on:`: Defines the events that trigger this workflow
  ...
  - `push.tags: "v*"`: Triggers when you push tags starting with "v" (e.g., v1.0.0, v2.1.3)

**Why both branches and tags?**

- **Branch pushes**: For continuous development—every commit builds new images
- **Version tags**: For releases—tag your code with v1.0.0 and get properly versioned images
...
- `type=semver,pattern={{version}}`: Full semantic version from git tags (e.g., `1.2.3`)
...
**Example:** When you tag a release as `v1.0.0`, you get:
- `ghcr.io/username/furnet/backend:1.0.0`
- `ghcr.io/username/furnet/backend:1.0`
- All the branch-based tags too

**Why multiple tags?**

Different deployment scenarios need different tags:
- `latest`: For development and testing
- `main-abc1234`: For tracking specific commits
- `1.0.0`: For production releases
...
- `ghcr.io/username/furnet/frontend:1.0.0`
...
6. **Metadata generation** → Creates appropriate tags based on branch/tag
...
This triggers the workflow again, and you'll get images tagged with:
- `1.0.0`
- `1.0`
- `latest`
- `main`
...
Check your packages page to see all the version tags!
...
- ✓ Implemented intelligent tagging (latest, versioned, commit-specific)
